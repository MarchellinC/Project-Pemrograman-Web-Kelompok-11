...
<body>
    ...
    <div class="container">
        <div class="link">
            <a href="<?= base_url('users/home') ?>">Home</a> >> <a href="<?= base_url('users/' . ($kendaraan['tipe'] === 'Mobil' ? 'listMobil' : 'listMotor')) ?>"><?= ucfirst($kendaraan['tipe']) ?></a> >> <a href="<?= base_url('users/deskripsi/' . $kendaraan['id_kendaraan']) ?>"><?= $kendaraan['merk'] ?></a> >> <a href="<?= current_url() ?>"> Payment </a>
        </div>
        <div class="content">
            ...
            <?php endif; ?>
                <form action="<?= base_url('users/formTransaksi/' . $kendaraan['id_kendaraan']) ?>" method="POST">
        ...
        function calculateTotal() {
            ...
            if (tanggalSewa && tanggalKembali) {
                ...
                if (kembaliDate <= sewaDate) {
                    ...
                    document.getElementById("total-payment").innerHTML = "Total Pembayaran: <strong>Rp. 0,00</strong>";
                    document.getElementById("hidden-total-payment").value = 0;
                    return; 
                }
